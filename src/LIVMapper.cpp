...
/**
 * @brief LIVMapper类构造函数，初始化激光-惯性-视觉里程计的核心组件
 *
 * 该构造函数是整个FAST-LIVO2系统的入口点，负责初始化所有核心组件、
 * 加载配置参数、分配内存资源等。
 *
 * @param nh ROS节点句柄，用于获取配置参数和创建订阅/发布者
 */
LIVMapper::LIVMapper(ros::NodeHandle &nh)
    : extT(0, 0, 0),        // 激光雷达到IMU的平移外参，初始化为零向量
      extR(M3D::Identity()) // 激光雷达到IMU的旋转外参，初始化为单位矩阵
{
  // 初始化外参向量，用于存储从配置文件读取的外参
  extrinT.assign(3, 0.0);       // 激光雷达到IMU的平移外参向量
  extrinR.assign(9, 0.0);       // 激光雷达到IMU的旋转外参矩阵(行优先存储)
  cameraextrinT.assign(3, 0.0); // 相机到IMU的平移外参向量
  cameraextrinR.assign(9, 0.0); // 相机到IMU的旋转外参矩阵(行优先存储)
  ...
  // 初始化各种点云指针，用于存储不同处理阶段的点云数据
  visual_sub_map.reset(new PointCloudXYZI());          // 视觉子图点云
  ...
  pcl_w_wait_pub.reset(new PointCloudXYZI());          // 等待发布的世界坐标系点云
  pcl_wait_pub.reset(new PointCloudXYZI());            // 等待发布的点云
  pcl_wait_save.reset(new PointCloudXYZRGB());         // 等待保存的RGB点云
  pcl_wait_save_intensity.reset(new PointCloudXYZI()); // 等待保存的带强度信息的点云

  // 创建体素地图管理器实例，用于管理体素地图的构建和维护
  voxelmap_manager.reset(new VoxelMapManager(voxel_config, voxel_map));
  // 创建VIO(视觉惯性里程计)管理器实例，用于处理视觉和惯性数据
...
void LIVMapper::initializeFiles()
{
  ...
  if (pcd_save_en)
    fout_lidar_pos.open(std::string(ROOT_DIR) + "Log/pcd/scans_lidar_pos.txt", std::ios::out);
  if (img_save_en)
    fout_visual_pos.open(std::string(ROOT_DIR) + "Log/image/scans_visual_pos.txt", std::ios::out);
...
void LIVMapper::imu_cbk(const sensor_msgs::Imu::ConstPtr &msg_in)
{
  ...
  // 打印成功放入buffer的第一帧IMU数据时间戳
  static bool first_imu_printed = false;
  if (!first_imu_printed)
  {
    ROS_INFO("Get IMU, its header time: %.6f", timestamp);
    first_imu_printed = true;
  }
...
bool LIVMapper::sync_packages(LidarMeasureGroup &meas)
{
  ...
  std::cout << "slam_mode: " << slam_mode_ << std::endl;
  switch (slam_mode_)
  {
    case ONLY_LIO:
    {
    ...
    // 以图像帧的时间戳为切割点，将 LiDAR 点云切开
    // 强行把连续的 LiDAR 扫描流，按照相机快门的时刻切断
    // VIO 和 LIO 两个状态间交替
    case LIVO:
    {
      ...
      switch (last_lio_vio_flg)
      {
        ...
        {
          // ========== VIO模式数据同步处理 ==========
          // 当有图像数据时，需要将LiDAR和IMU数据与图像时间戳对齐
          // 计算图像捕获时间（图像时间戳 + 曝光时间补偿）
          double img_capture_time = img_time_buffer.front() + exposure_time_init;
          
          // 初始化上次LIO更新时间
          // 如果是第一次处理，将上次更新时间设置为第一帧LiDAR的时间戳
          if (meas.last_lio_update_time < 0.0)
            meas.last_lio_update_time = lid_header_time_buffer.front();

          // 计算当前缓冲区中最新的LiDAR和IMU数据时间戳
          // LiDAR最新时间 = 最后一帧LiDAR扫描中最后一个点的时间戳
          // （帧起始时间 + 该点相对帧起始的时间偏移）
          ...
          // 检查1: 图像时间戳是否早于上次LIO更新时间
          // 如果是，说明这是一个过时的图像帧，需要丢弃
          ...
          // 检查2: 图像时间戳是否超过当前可用的LiDAR或IMU数据范围
          // 如果是，说明需要等待更多的传感器数据到达
          ...
          m.imu.clear();
          m.lio_time = img_capture_time;  // 设置LIO处理的目标时间为图像捕获时间
          ...
          while (!imu_buffer.empty())
          {
            ...
            // 只提取在[last_lio_update_time, img_capture_time]时间段内的IMU数据
          ...
          mtx_buffer.unlock();
          sig_buffer.notify_all();  // 通知其他线程缓冲区已更新

          // ========== 准备点云处理缓冲区 ==========
          // 将上次处理剩余的点云(pcl_proc_next)移动到当前处理缓冲区(pcl_proc_cur)
          *(meas.pcl_proc_cur) = *(meas.pcl_proc_next);
          PointCloudXYZI().swap(*meas.pcl_proc_next);  // 清空next缓冲区
          ...
          // ========== 处理LiDAR点云数据 ==========
          // 将点云按时间戳分配到当前帧(cur)或下一帧(next)
          while (!lid_raw_data_buffer.empty())
          {
            // 如果LiDAR帧的起始时间超过图像时间，停止处理
            ...
            for (int i = 0; i < pcl.size(); i++)
            {
              ...
              // 根据点的时间戳决定分配到哪个缓冲区
              // curvature字段存储的是点相对于帧起始时间的偏移(ms)
              if (pcl[i].curvature < max_offs_time_ms)
              {
                // 点的时间戳在[last_lio_update_time, img_capture_time]范围内
                // 分配到当前处理缓冲区，并更新时间偏移
          ...
          // ========== 完成数据组装 ==========
          meas.measures.push_back(m);      // 添加测量数据组
          meas.lio_vio_flg = LIO;          // 设置标志为LIO模式（需要进行LIO更新）
          
          return true;  // 数据准备完成，可以进行处理
        }
