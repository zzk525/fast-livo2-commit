...
/**
 * @brief Preprocess类构造函数，初始化激光雷达数据预处理的各种参数
 *
 * 该构造函数设置了激光雷达数据预处理所需的默认参数，包括传感器类型、
 * 特征提取参数、距离阈值、角度阈值等。这些参数用于点云数据的过滤、
 * 特征提取和分类等预处理操作。
 */
Preprocess::Preprocess() : feature_enabled(0), // 特征提取开关，0表示关闭特征提取
                           lidar_type(AVIA),   // 激光雷达类型，默认使用Livox AVIA激光雷达
                           blind(0.01),        // 盲区阈值，小于该距离的点将被过滤掉（单位：米）
                           point_filter_num(1) // 点云滤波间隔，1表示不进行降采样滤波
{
  // 无穷大边界值，用于判断异常点
  inf_bound = 10;
  // 扫描线数，默认设置为6线激光雷达
  N_SCANS = 6;
  // 点云分组大小，用于特征提取时的点云分组处理
  group_size = 8;
  // 距离阈值A，用于点云特征判断
  disA = 0.01;
  // // 注意：这里可能存在笔误，应该是disB = 0.1; 从代码逻辑看，这两个应该是不同的距离阈值
  // disA = 0.1; // B?
  disB = 0.1;
  // 点到线的比例阈值，用于平面特征判断
  p2l_ratio = 225;
  ...
  edgea = 2;
  edgeb = 0.1;
  // 小平面相交的角度阈值（度），用于判断小平面特征
  smallp_intersect = 172.5;
  // 小平面比例阈值，用于判断小平面特征
  smallp_ratio = 1.2;
  // 是否给定时间偏移，false表示需要自动计算时间偏移
  given_offset_time = false;

  // 将角度值（度）转换为弧度后计算余弦值，用于快速角度比较
  // 这样可以避免在实时处理中频繁计算余弦值
